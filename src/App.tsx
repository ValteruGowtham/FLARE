...
export default function App() {
  ...
  // UI state
  const [activeView, setActiveView] = useState<"tasks" | "habits" | "insights">(
    "tasks",
  );
  ...
  const [guestMergeTasks, setGuestMergeTasks] = useState<any[] | null>(null);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (firebaseUser) => {
      if (firebaseUser) {
        ...
        const guestTasksJson = localStorage.getItem("flare_guest_tasks");
        const guestTasks = guestTasksJson ? JSON.parse(guestTasksJson) : [];
        if (
          guestTasks.length > 0 &&
          savedGuest === "true" &&
          !localStorage.getItem("guest_merge_prompted_" + firebaseUser.uid)
        ) {
          setGuestMergeTasks(guestTasks);
        }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      console.log(
        "Running periodic risk scoring re-evaluation with calendar support...",
      );
      const userId = user ? user.uid : null;
      try {
        ...
        if (token) {
          ...
          if (activeTasks.length > 0) {
            ...
            const busyIntervals =
              await fetchBusyIntervalsForAllTasks(deadlines);
            const taskBusyIntervalsMap: Record<
              string,
              Array<{ start: string; end: string }>
            > = {};
            activeTasks.forEach((task) => {
              const taskDeadlineTime = new Date(task.deadline).getTime();
              taskBusyIntervalsMap[task.id] = busyIntervals.filter(
                (interval) => {
  ...
  const fetchBusyIntervalsForAllTasks = async (
    deadlines: string[],
  ...
  const handleMergeGuestData = async () => {
    if (!guestMergeTasks || !user) return;
    setLoading(true);
    try {
      // Create each guest task for the new user
      for (const t of guestMergeTasks) {
      ...
      localStorage.removeItem("flare_guest_tasks");
      localStorage.setItem("guest_merge_prompted_" + user.uid, "true");
      setGuestMergeTasks(null);
      setToast({
        message: `Successfully imported ${guestMergeTasks.length} tasks!`,
      });
      // Reload tasks
      const loadedTasks = await fetchTasks(user.uid);
      setTasks(loadedTasks);
    } catch (err) {
  ...
  const handleConnectCalendar = async () => {
    if (!user) {
      alert(
        "Please Sign In with an account to connect Google Calendar & Gmail.",
      );
      return;
    }
  ...
  const confirmConnectCalendar = async () => {
    ...
    try {
      ...
      if (token) {
        ...
        setToast({
          message:
            "Google Calendar & Gmail connected successfully! Rescanning deadlines...",
        });
        // Recalculate
  ...
  const handleDisconnectCalendar = async () => {
    ...
    if (
      window.confirm(
        "Disconnect Google Calendar? This will disable smart schedule-aware risk scoring and autonomous extensions.",
      )
    ) {
      try {
  ...
  const handleDeleteAccount = async () => {
    ...
    if (
      window.confirm(
        "Are you entirely sure you want to delete your account? This action is permanent and will delete all tasks, habits, and revoke calendar access immediately.",
      )
    ) {
      try {
        const { db } = await import('./firebase');
        const { collection, query, where, getDocs, doc, deleteDoc, writeBatch } = await import('firebase/firestore');

        let tokenToRevoke = '';
        const tokenDocRef = doc(db, 'user_tokens', user.uid);
        const tokenDoc = await (await import('firebase/firestore')).getDoc(tokenDocRef);
        if (tokenDoc.exists()) {
        ...
        await handleLogout();
        setToast({ message: "Account deleted successfully." });
      } catch (err) {
  ...
  const handleRunSweep = async () => {
    ...
    try {
      if (!user) {
        throw new Error("Must be logged in to run sweep.");
      }
      ...
      const response = await fetch("/api/dev/sweep", { 
        method: "POST",
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (data.taskUpdatesMap) {
        for (const taskId of Object.keys(data.taskUpdatesMap)) {
          const updates = data.taskUpdatesMap[taskId];
          await updateDoc(doc(db, 'tasks', taskId), updates).catch(console.error);
        }
      }
      ...
      const refreshedTasks = await fetchTasks(user.uid);
  ...
  const handleScheduleTask = async (task: Task) => {
    ...
    try {
      ...
      const latestToken = getCachedAccessToken() || token;
      const eventDetails = await createCalendarEvent(
        ...
        slot.end,
        task.description ||
          "Focused work block scheduled automatically by Flare AI.",
      );
  ...
  const handleDrop = async (
    e: DragEvent,
    newRiskScore: "Critical" | "Urgent" | "Stable",
  ) => {
    e.preventDefault();
    ...
    setTasks((prev) =>
      prev.map((t) =>
        t.id === taskId
          ? { ...t, riskScore: newRiskScore, manualRiskOverride: true }
          : t,
      ),
    );
        ...
        prev.map((t) =>
          t.id === taskId
            ? {
                ...
                manualRiskOverride: task.manualRiskOverride,
              }
            : t,
        ),
      );
  ...
  const handleStatusChange = async (taskId: string, newStatus: TaskStatus) => {
    ...
    try {
      ...
      if (newStatus !== "done" && token) {
        ...
        if (existingTask) {
          try {
            ...
            console.error(
              "Failed to pre-fetch free/busy for status change:",
              err,
            );
          }
        }
      ...
      const updated = await updateTask(
        ...
        taskId,
        { status: newStatus },
        token && newStatus !== "done" ? busyIntervals : undefined,
      );
  ...
  const criticalTasks = filteredTasks.filter(
    (t) => t.status !== "done" && t.riskScore === "Critical",
  );
  const urgentTasks = filteredTasks.filter(
    (t) => t.status !== "done" && t.riskScore === "Urgent",
  );
  const stableTasks = filteredTasks.filter(
    (t) => t.status !== "done" && t.riskScore === "Stable",
  );
  const finishedTasks = filteredTasks.filter((t) => t.status === "done");

  // Compute column metrics
  const criticalHours = criticalTasks.reduce(
    (sum, t) => sum + t.estimatedEffort,
    0,
  );
  const urgentHours = urgentTasks.reduce(
    (sum, t) => sum + t.estimatedEffort,
    0,
  );
  const stableHours = stableTasks.reduce(
    (sum, t) => sum + t.estimatedEffort,
    0,
  );
  const finishedHours = finishedTasks.reduce(
    (sum, t) => sum + t.estimatedEffort,
    0,
  );
  ...
  if (authChecking) {
    return (
      <div
        className="min-h-screen flex flex-col items-center justify-center bg-[#F9F8F6] dark:bg-zinc-950 dark:text-zinc-100 px-6"
        id="splash-screen"
      >
        <div className="flex flex-col items-center text-center max-w-sm space-y-4">
          <h1 className="text-6xl font-black italic tracking-tighter font-serif text-zinc-950 dark:text-white">
            FLARE.
          </h1>
          <div className="h-px w-16 bg-black/25 dark:bg-white/25" />
          <p className="text-[10px] uppercase tracking-[0.2em] font-bold text-zinc-500 dark:text-zinc-400">
            AI Triage engine is warming up...
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div
      className="min-h-screen bg-[#F9F8F6] text-[#1A1A1A] dark:bg-zinc-950 dark:text-zinc-100 flex flex-col px-4 sm:px-8 md:px-12 py-8"
      id="app-root-container"
    >
      {/* 1. TOP MAIN NAV / HEADER */}
      <header
        className="flex flex-col lg:flex-row justify-between items-start border-b border-black/10 dark:border-white/10 pb-8 mb-8 gap-6"
        id="main-header"
      >
        {/* Logo Brand & Metadata */}
        <div className="flex flex-col">
          <div className="flex items-center gap-4">
            <h1 className="text-5xl font-black tracking-tighter leading-none italic font-serif">
              FLARE.
            </h1>
            <button
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2">
            <p className="text-[10px] uppercase tracking-[0.2em] font-bold opacity-60">
              AI Deadline Rescue Companion
            </p>
            <span className="text-[9px] font-mono bg-black/5 dark:bg-white/5 text-zinc-700 dark:text-zinc-300 px-1.5 py-0.5 border border-black/5 dark:border-white/5">
              v1.2-AI
            </span>
            ...
            {calendarConnected ? (
              <span
                className="text-[9px] font-mono font-bold bg-emerald-100 text-emerald-800 px-1.5 py-0.5 border border-emerald-200 flex items-center gap-0.5"
                id="calendar-header-status-badge"
              >
                📅 Calendar Active
              </span>
            ) : (
            ...
            {user && (
              <>
                <button
                  ...
                  className="text-[9px] font-bold uppercase tracking-wider text-zinc-600 hover:text-black dark:text-zinc-400 dark:hover:text-white hover:underline cursor-pointer ml-1"
            ...
            {!user && isGuest && (
              <button
                ...
                className="text-[9px] font-bold uppercase tracking-wider text-zinc-500 hover:text-black dark:text-zinc-400 dark:hover:text-white hover:underline cursor-pointer ml-1 transition-colors"
        ...
        {pinnedTask ? (
          <div
            className="bg-white dark:bg-zinc-900 border border-red-200 dark:border-red-500/20 p-5 rounded-none flex flex-col md:flex-row md:items-center gap-6 max-w-2xl shadow-xs animate-fade-in"
            id="pinned-hero-banner"
          >
            ...
            <div className="flex-1">
              <h2 className="text-sm font-extrabold leading-tight text-zinc-950 font-sans">
                {pinnedTask.title}
              </h2>
              <p className="text-[10px] opacity-70 mt-1 font-mono">
                ⏱️ {pinnedTask.estimatedEffort}h effort • Due{" "}
                {new Date(pinnedTask.deadline).toLocaleDateString(undefined, {
                  month: "short",
                  day: "numeric",
                  hour: "2-digit",
                  minute: "2-digit",
            ...
            <div className="md:pl-6 md:border-l border-black/5 flex flex-col items-start md:items-end text-left md:text-right max-w-sm">
              <p className="text-[9px] font-bold opacity-40 uppercase tracking-widest mb-1">
                AI Diagnostic
              </p>
            ...
            <div className="flex md:flex-col gap-2 shrink-0 pt-2 md:pt-0 border-t md:border-t-0 md:pl-4 border-black/5">
              {pinnedTask.status === "not started" ? (
                <button
                  onClick={() =>
                    handleStatusChange(pinnedTask.id, "in progress")
                  }
          ...
          <div
            className="bg-white dark:bg-zinc-900 border border-black/5 dark:border-white/5 p-4 rounded-none flex items-center gap-4 max-w-xl shadow-xs"
            id="clear-hero-banner"
          >
            ...
            <div>
              <h2 className="text-xs font-bold leading-tight">
                No Urgent Threats Outstanding
              </h2>
              <p className="text-[11px] opacity-60 mt-0.5">
                All tracked tasks are completed or comfortably padded.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="flex gap-6 self-center lg:self-auto">
            <div className="text-center">
              <p className="text-3xl font-black">
                {criticalTasks.length.toString().padStart(2, "0")}
              </p>
            ...
            <div className="text-center">
              ...
              <p className="text-[9px] uppercase tracking-wider font-bold text-orange-500">
                Urgent
              </p>
            </div>
            <div className="text-center">
              <p className="text-3xl font-black">
                {stableTasks.length.toString().padStart(2, "0")}
              </p>
            ...
            <div className="text-center">
              <p className="text-3xl font-black">
                {finishedTasks.length.toString().padStart(2, "0")}
              </p>
      ...
      <main className="flex-1 flex flex-col space-y-6" id="dashboard-content">
        {activeView === "habits" ? (
          ...
          <>
            ...
            <div
              className="bg-white dark:bg-zinc-900 border border-black/10 dark:border-white/10 p-4 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 shadow-2xs"
              id="board-filters-bar"
            >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold text-zinc-900 uppercase tracking-widest">
                      Connect Calendar for smarter risk scoring
                    </p>
                    <p className="text-[11px] text-zinc-600 mt-1 leading-relaxed">
                      Authorize Flare to scan your scheduled meetings. We'll
                      compute your actual available hours (subtracting calendar
                      busy blocks) to precisely assess your deadline risks.
                    </p>
                  </div>
                </div>
                <button
            ...
            {(!loading || tasks.length > 0) && (
              <div
                className={`grid grid-cols-1 md:grid-cols-2 ${showDone ? "xl:grid-cols-4 lg:grid-cols-3" : "lg:grid-cols-3"} gap-8 items-start ${loading ? "opacity-70 pointer-events-none" : ""}`}
                id="triage-board-columns"
              >
                {/* COLUMN 1: CRITICAL */}
                <section className="flex flex-col gap-4" id="col-critical">
                  ...
                  <div className="flex justify-between items-baseline mb-2 border-b-2 border-red-500 pb-2">
                    <div className="flex items-baseline gap-2">
                      <h3 className="text-lg font-black uppercase tracking-tighter text-zinc-900 dark:text-zinc-100">
                        01. Critical
                      </h3>
                      <span className="text-xs font-bold font-mono text-red-600 dark:text-red-500">
                        ({criticalTasks.length})
                      </span>
                    </div>
                    <span className="text-[10px] font-mono opacity-50 italic">
                      At Risk ({criticalHours}h)
                    </span>
                  </div>
                  ...
                  >
                    {criticalTasks.length > 0 ? (
                      ...
                      <div className="flex-1 border border-dashed border-black/10 dark:border-white/10 rounded-none bg-white/40 dark:bg-white/5 flex flex-col items-center justify-center p-8 text-center text-zinc-400 dark:text-zinc-500">
                        <p className="text-xs font-bold uppercase tracking-wider text-zinc-800 dark:text-zinc-300">
                          No Critical Threats
                        </p>
                        <p className="text-[10px] mt-1 max-w-[180px] font-sans">
                          All active items have a comfortable timeline buffer.
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  >
                    {urgentTasks.length > 0 ? (
                      ...
                      <div className="flex-1 border border-dashed border-black/10 dark:border-white/10 rounded-none bg-white/40 dark:bg-white/5 flex flex-col items-center justify-center p-8 text-center text-zinc-400 dark:text-zinc-500">
                        ...
                        <p className="text-[10px] mt-1 max-w-[180px] font-sans">
                          Nothing pressing due today or tomorrow.
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  >
                    {stableTasks.length > 0 ? (
                      ...
                      <div className="flex-1 border border-dashed border-black/10 dark:border-white/10 rounded-none bg-white/40 dark:bg-white/5 flex flex-col items-center justify-center p-8 text-center text-zinc-400 dark:text-zinc-500">
                        ...
                        <p className="text-[10px] mt-1 max-w-[180px] font-sans">
                          Deploy tasks to watch their buffer values change.
                        </p>
                      </div>
                    )}
                  </div>
      ...
      {/* 5. FOOTER */}
      <footer
        className="mt-auto flex flex-col sm:flex-row justify-between items-center pt-8 mt-12 border-t border-black/10 gap-4 text-[11px]"
        id="main-footer"
      >
      ...
      {guestMergeTasks && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-zinc-900 border border-black/10 dark:border-white/10 w-full max-w-md p-8 shadow-2xl relative">
            <h2 className="text-2xl font-black italic tracking-tighter mb-4 text-zinc-900 dark:text-zinc-100">
              Found {guestMergeTasks.length} offline tasks.
            </h2>
            <p className="text-zinc-600 dark:text-zinc-400 text-sm leading-relaxed mb-8">
              You have tasks saved from your guest session. Would you like to
              bring them into your account?
            </p>
            <div className="flex gap-4">
              <button
                onClick={handleDiscardGuestData}
                className="flex-1 px-4 py-3 border border-black/10 dark:border-white/10 hover:bg-zinc-50 dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-xs font-bold uppercase tracking-widest transition"
              >
                Discard
              </button>
              <button
                ...
                disabled={loading}
                className="flex-1 px-4 py-3 bg-black dark:bg-white text-white dark:text-black hover:bg-zinc-800 dark:hover:bg-zinc-200 text-xs font-bold uppercase tracking-widest transition disabled:opacity-50"
              >
      ...
      {isConsentOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-zinc-900 border border-black/10 dark:border-white/10 w-full max-w-lg p-8 shadow-2xl relative">
            <div className="absolute top-4 right-4 p-2 bg-amber-100 border border-amber-200">
              <Shield className="w-5 h-5 text-amber-700" />
            </div>
            <h2 className="text-2xl font-black italic tracking-tighter mb-4 pr-12 text-zinc-900 dark:text-zinc-100">
              Connect Google Calendar & Gmail
            </h2>
            <div className="space-y-4 mb-8">
              <p className="text-zinc-600 dark:text-zinc-400 text-sm leading-relaxed">
                To provide smart schedule-aware risk scoring and autonomous
                extensions, Flare requests access to your Google account. Here
                is exactly what we use it for:
              </p>
              <ul className="text-xs text-zinc-600 dark:text-zinc-400 space-y-3 list-disc pl-4">
                <li>
                  <strong>Read your free/busy schedule:</strong> Used to compute
                  your actual available hours (subtracting calendar busy blocks)
                  to precisely assess your deadline risks.
                </li>
                <li>
                  <strong>Create Calendar Events:</strong> Used to schedule
                  dedicated "focus blocks" directly onto your calendar when a
                  task is at risk.
                </li>
                <li>
                  <strong>Send Draft Emails:</strong> Used by the Rescue Agent
                  to send automated extension requests to your contacts when a
                  deadline cannot be met.
                </li>
              </ul>
              <p className="text-zinc-500 text-xs italic mt-4">
                This is completely optional and can be revoked at any time from
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                className="flex-1 px-4 py-3 bg-black dark:bg-white text-white dark:text-black hover:bg-zinc-800 dark:hover:bg-zinc-200 text-xs font-bold uppercase tracking-widest transition flex items-center justify-center gap-2"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-zinc-900 border border-black/10 dark:border-white/10 w-full max-w-md p-8 shadow-2xl relative">
            <button
              ...
              className="absolute top-4 right-4 p-2 text-zinc-400 hover:text-black dark:hover:text-white transition"
            >
              <span className="text-lg leading-none">&times;</span>
            </button>
            <h2 className="text-2xl font-black italic tracking-tighter mb-8 border-b border-black/10 dark:border-white/10 pb-4 text-zinc-900 dark:text-zinc-100">
              Account Settings
            </h2>
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-[9px] font-extrabold text-zinc-500 uppercase tracking-widest mb-2">
                  Google Integrations
                </label>
                {calendarConnected ? (
                  <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800/50 rounded-sm">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-2 text-emerald-800 dark:text-emerald-400 text-sm font-bold mb-1">
                    ...
                    <p className="text-xs text-emerald-700 dark:text-emerald-500 leading-relaxed mt-2">
                      Calendar and Gmail features are currently active and
                      enabling smart diagnostics.
                    </p>
                  </div>
                ) : (
                  <div className="p-4 bg-zinc-50 dark:bg-zinc-800/50 border border-black/10 dark:border-white/10 rounded-sm">
                    <p className="text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed mb-3">
                      Connect your Google account to unlock schedule-aware
                      deadline risk scoring and autonomous agent features.
                    </p>
                    <button
                      onClick={handleConnectCalendar}
                      className="text-[10px] bg-black dark:bg-white text-white dark:text-black hover:bg-zinc-800 dark:hover:bg-zinc-200 px-3 py-1.5 font-bold uppercase tracking-widest transition"
                    >
                      Connect Calendar
                    </button>
                  </div>
              ...
              <div className="pt-6 border-t border-black/10 dark:border-white/10 mt-6">
                <label className="block text-[9px] font-extrabold text-red-500 uppercase tracking-widest mb-2">
                  Danger Zone
                </label>
                ...
                <button
                  ...
                  className="w-full px-4 py-2.5 border border-red-200 bg-red-50 hover:bg-red-100 text-red-700 text-xs font-bold uppercase tracking-widest transition flex items-center justify-center gap-2"
                >
